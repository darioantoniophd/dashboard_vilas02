...
Plot A10 vs prueba, color según rango de QC1 (tres rangos)
```{r}
...
Tabla para analizar frecuencia y tipo de problemas
```{r}

problemas <- data.frame(table(my_data$Problema))
porcentaje <- c(1:16)
problemas <- cbind(problemas, porcentaje)
colnames(problemas) = c("Tipo de problema", "N", "frecuencia")
sum(problemas$N)
problemas$frecuencia <- problemas$N*100/sum(problemas$N)
problemas$frecuencia <-  sprintf("%.1f", problemas$frecuencia)
```
