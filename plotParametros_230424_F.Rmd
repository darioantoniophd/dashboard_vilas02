...
ggplot(subset_data, aes(x = Prueba, y =`A10 (600s+CT) [Pa]`, col = colores[rangos_QC1])) +
  ...
  #scale_fill_discrete(guide = guide_legend(title = "Rangos de QC1")) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
ggplot(subset_data, aes(x = Prueba, y =`A10 (600s+CT) [Pa]`, col = colores[rangos_QC1])) +
  ...
  labs(x = "Pruebas", y = "A10") +
  ggtitle("A10 vs. Prueba coloreado según rangos de QC1") +
  scale_fill_discrete(guide = guide_legend(title = "Rangos QC1", title.position = "top", override.aes = list(size = 3))) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
