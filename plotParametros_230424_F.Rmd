...
#box plot con CV y N
ggplot(subset_data, aes(x = subset_data$`Nro Cartucho`, y = `A10 (600s+CT) [Pa]`)) +
  geom_boxplot() +
  ...
  geom_text(data = cv_cartuchos, aes(x = N_cartucho$`Nro Cartucho`, y = max(extem$`A10 (600s+CT) [Pa]`), label = paste0("", cv, "%")), vjust = 1, angle =45, size = 3, color = "blue", nudge_x = 0.2)

#subset de mediciones con dato de cartucho????
