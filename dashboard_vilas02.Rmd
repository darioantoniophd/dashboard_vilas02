...
} else if (plot_type == "3_sigma_A_por_prueba"){
  # boxplot with labels
  # calculate CV values for each group
  cv_data <- midata %>%
  group_by(Prueba) %>%
  summarise(promedio=mean(.data[[a_col]]),sigmax3 = sd(.data[[a_col]]),muestra=max(Muestra),activacion=max(Activacion))
...
circularise <- function(d, n=30){
  ...
  lmat <- mapply(make_circle, id = seq_len(nrow(d)), 
                x = 0, y=d[["promedio"]], r=d[["sigmax3"]],circle_color=d[["muestra"]], SIMPLIFY = FALSE)
  do.call(rbind, lmat)
}
...
selectInput("plot_type", "Select plot type:", choices = c('scatter_A_vs_Prueba','boxplot_por_prueba','boxplot_por_muestra','scatter_A_vs_Pmed','scatter_A_vs_Pruptura','3_sigma_A_por_prueba'), multiple = FALSE,selected ='scatter')
...
sliderInput("range_deltaPmed", "Select a range of values:", min=min_val, max=max_val, value=c(min_val, max_val))
