 ...
 if (plot_type == "scatter_A_vs_Prueba") {
  ...
  ggplot(midata, aes(x = Prueba, y = .data[[a_col]], color = .data[[color_col]], shape = .data[[shape_col]])) +
   ...
   # annotation_raster(img, xmin = 10, xmax = 30, ymin = 1000, ymax = 1400)
...
} else if (plot_type == "scatter_A_vs_Medicion"){
    ggplot(midata, aes(x = Medicion, y = .data[[a_col]], color = Prueba, shape = .data[[shape_col]])) +
    geom_point(size= 3) +
    labs(title = paste0(a_col, " vs. ", "Medicion"), x = "Medicion", y = a_col, color = color_col, shape=shape_col)+
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
  
 } else if (plot_type == "scatter_A_vs_Pmed"){
    ggplot(midata, aes(x = deltaPmed, y = .data[[a_col]], color = Prueba, shape = .data[[shape_col]])) +
    geom_point(size= 3) +
    ...
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
} else if (plot_type == "scatter_A_vs_canal_abierto"){
    ggplot(midata, aes(x = `Canal abierto Promedio QC2 [Pa]`, y = .data[[a_col]], color = .data[[color_col]], shape = .data[[shape_col]])) +
    geom_point(size= 3) +
    labs(title = paste0(a_col, " vs. ", "Canal abierto Promedio QC2 [Pa]"), x = "Canal abierto Promedio QC2 [Pa]", y = a_col, color = color_col, shape=shape_col)+
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
} else if (plot_type == "scatter_A_vs_Amplitud_base"){
    ggplot(midata, aes(x = `Amplitud_base`, y = .data[[a_col]], color = .data[[color_col]], shape = .data[[shape_col]])) +
    geom_point(size= 3) +
    labs(title = paste0(a_col, " vs. ", "Amplitud base"), x = "Amplitud base", y = a_col, color = color_col, shape=shape_col)+
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
} else if (plot_type == "canal_cerrado_vs_medicion"){
    ggplot(midata, aes(x = Medicion, y = `Canal cerrado Promedio QC1 [Pa]`, color = .data[[color_col]], shape = .data[[shape_col]])) +
    geom_point(size= 3) +
    labs(title = paste0("Canal cerrado Promedio [Pa]", " vs. ", "Medicion"), x = "Medicion", y = "Canal cerrado", color = color_col, shape=shape_col)+
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
} else if (plot_type == "canal_abierto_vs_medicion"){
    ggplot(midata, aes(x = Medicion, y =  `Canal abierto Promedio QC2 [Pa]`, color = .data[[color_col]], shape = .data[[shape_col]])) +
    geom_point(size= 3) +
    labs(title = paste0("Canal abierto", " vs. ", "Medicion"), x = "Medicion", y = "Canal abierto", color = color_col, shape=shape_col)+
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
} else if (plot_type == "Amplitud_base_vs_medicion"){
    ggplot(midata, aes(x = Medicion, y = Amplitud_base, color = .data[[color_col]], shape = .data[[shape_col]])) +
    geom_point(size= 3) +
    labs(title = paste0("Amplitud_base", " vs. ", "Medicion"), x = "Medicion", y = "Amplitud_base", color = color_col, shape=shape_col)+
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
} else if (plot_type == "Amplitud_base_vs_fecha"){
    ggplot(midata, aes(x = Fecha, y = Amplitud_base, color = .data[[color_col]], shape = .data[[shape_col]])) +
    geom_point(size= 3) +
    labs(title = paste0("Amplitud_base", " vs. ", "Fecha"), x = "Fecha", y = "Amplitud_base", color = color_col, shape=shape_col)+
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))     
...
selectInput("value_amplitud", "Select Amplitude:", choices = c('A1 (60s+CT) [Pa]','A10 (600s+CT) [Pa]','A10R','A5 (300s+CT) [Pa]'), multiple = FALSE, selected ='A10 (600s+CT) [Pa]')
selectInput("values_freq", "Select values for Frequency:", choices = unique(subset_data$Frecuencia_Hz), multiple = TRUE, selected = unique(subset_data$Frecuencia_Hz))
selectInput("plot_type", "Select plot type:", choices = c('scatter_A_vs_Prueba','scatter_A_vs_Medicion','boxplot_por_prueba','boxplot_por_muestra','scatter_A_vs_Pmed','scatter_A_vs_Pruptura','sigma_A_por_prueba','scatter_A_vs_canal_abierto','canal_cerrado_vs_medicion','scatter_A_vs_Amplitud_base','Amplitud_base_vs_medicion','Amplitud_base_vs_fecha','canal_abierto_vs_medicion'), multiple = FALSE,selected ='scatter')
