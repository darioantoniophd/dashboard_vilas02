 ...
 if (plot_type == "scatter_A_vs_Prueba") {
  ...
  ggplot(midata, aes(x = Prueba, y = .data[[a_col]], color = .data[[color_col]], shape = .data[[shape_col]])) +
   ...
   # annotation_raster(img, xmin = 10, xmax = 30, ymin = 1000, ymax = 1400)
    ...
    geom_point(size= 3) +
    labs(title = paste0("Canal abierto Promedio QC2 [Pa]", " vs. ", "Medicion"), x = "Medicion", y = "Canal abierto", color = color_col, shape=shape_col)+
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
selectInput("plot_type", "Select plot type:", choices = c('scatter_A_vs_Prueba','scatter_A_vs_Medicion','boxplot_por_prueba','boxplot_por_muestra','scatter_A_vs_Pmed','scatter_A_vs_Pruptura','sigma_A_por_prueba','scatter_A_vs_canal_abierto','canal_abierto_vs_medicion'), multiple = FALSE,selected ='scatter')
