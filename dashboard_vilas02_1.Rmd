...
scatter = {
  ...
  ggplot(midata, aes(x = .data[[x_col]], y = .data[[a_col]], color = .data[[color_col]], shape = .data[[shape_col]],text=Medicion)) +
    geom_point(size= 3) +
...
boxplot = {
  ...
  geom_text(data = N_data, aes(x = .data[[x_col]], y = min(midata[[a_col]])*.7, label = paste0("", N)),  size = 3, color = "red") +
  geom_text(data = cv_data, aes(x = .data[[x_col]], y = min(midata[[a_col]])*.9, label = paste0("", round(cv,0), "%")),  angle = 0, size = 3, color = "blue") 
} ,
boxplot_sd = {
  # boxplot with labels
  # calculate CV values for each group
...
# Plot boxplot with CV and N labels
  ...
  common_theme +
  geom_text(data = N_data, aes(x = .data[[x_col]], y = min(midata[[a_col]])*.7, label = paste0("", N)),  size = 3, color = "red") +
  #geom_text(data = cv_data, aes(x = .data[[x_col]], y = min(midata[[a_col]])*1.15, label = paste0("", round(sd,0))),  angle = 0, size = 3, color = "brown")+
  geom_text(data = cv_data, aes(x = .data[[x_col]], y = min(midata[[a_col]])*.9, label = paste0("", round(cv,0), "%")),  angle = 0, size = 3, color = "blue") #+
...
```{r}
num_variables <- sort(c('Prueba','Medicion','Fecha','A1','A5','A10','A10_mm','A20','CT','Amplitud_base','P_canal_abierto_QC2','deltaCC','P_canal_cerrado_QC1','P_canal_cerrado_QC3','P_desprendimiento','CT_R','A10_R','Pmed_base','deltaPmed','Alpha','Fase_A10','Tz_i','Tz_A10','deltaTz','A10corregido','A10_mm_corregido','Fila'))
...
renderPlotly({
    ...
    my_plot(midata_status(), x_col = input$x_var, a_col = input$y_var, color_col = input$color_var,  shape_col = input$shape_var,plot_type=input$plot_type)
  })
