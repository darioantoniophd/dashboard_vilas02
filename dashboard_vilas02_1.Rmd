...
subset_data_problema <- subset(my_data, Problema != "ninguno")
...
# Filtra las mediciones sin problemas según selección de la gui
reactive_subset_data_problema <- reactive({
  # Apply common filtering using the function
  filtered_data <- applyCommonFilter(subset_data_problema, input)
  return(filtered_data)
})
...
incluyeProblemas = TRUE
  renderPlot({
    midata <- switch(input$problemas,
    Good_and_failed = reactive_my_data(),
    Failed = reactive_subset_data_problema(),
    Good = reactive_subset_data())
