...
my_plot <- function(midata, x_col, a_col, color_col, shape_col, text_col, plot_type) {
# Read the JPEG image
...
scatter = {
  ...
  ggplot(midata, aes(x = .data[[x_col]], y = .data[[a_col]], color = .data[[color_col]], shape = .data[[shape_col]],text=.data[[text_col]])) +
    geom_point(size= 3) +
...
renderPlotly({
    ...
    my_plot(midata_status(), x_col = input$x_var, a_col = input$y_var, color_col = input$color_var,  shape_col = input$shape_var,text_col ='Medicion',plot_type=input$plot_type)
  })
